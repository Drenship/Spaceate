...
type PropsOption = {
    option: OptionType,
    setInputValue: () => Dispatch<SetStateAction<OptionType>>
}

function Option({ option, setInputValue }: PropsOption) {
    const { name } = option;

    console.log(option)

    return (
        <button className='w-full' onClick={() => setInputValue(option)}>
...
export default function InputSelect({ input, options }: PropsInput) {
    ...
    const [inputValue, setInputValue] = useState<Dispatch<SetStateAction<OptionType>>>(defaultValue);
