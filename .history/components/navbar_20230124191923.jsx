    ...
    return (
        <nav className="fixed top-0 z-50 justify-between h-16 space-x-4 shadow-lg navbar bg-base-100 md:px-8">
            ...
            <div className='relative border border[#f3f6fd] bg-[#f3f6fd] rounded-full shadow-lg w-full max-w-[480px] h-10 flex justify-between items-center px-3'>
                ...
                <div className='absolute left-0 right-0 -z-[1] w-full p-2 bg-[#f3f6fd] rounded-b-lg shadow-lg top-5'>
                    <div className='mt-5 space-y-1 overflow-hidden rounded-md'>
                        {
                            database.products.map((data, key) => (
                                <div className='flex w-full bg-white' key={key}> 
                                    <div className='relative flex-grow-0 h-12 aspect-square'>
                                        <BlurImage src={data.main_image} alt={data.name} />
                                    </div>
                                    <div className='flex-shrink w-full p-2'>
                                        <h3 className='text-sm font-semibold uppercase'>{data.name}</h3>
                                    </div>
                                </div>
                            ))
                        }
