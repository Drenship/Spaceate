...
export default function Navbar() {
    ...
    const isLogged = useMemo(() => {
        return status !== 'loading' && session?.user ? true : false
    }, [session, status]);
    ...
    return (
        <nav className="fixed top-0 z-50 justify-between h-16 space-x-4 shadow-lg navbar bg-base-100 md:px-8">
            ...
            <div className="flex-none space-x-4">
                ...
                {
                    isLogged === true ? (<div className="dropdown dropdown-end">
                    ...
                    </div>) : <Link href="/auth/login" className='hidden px-4 py-2 text-sm font-bold uppercase border border-black rounded lg:block button-click-effect'>Se connecter</Link>
                }
            </div>
        </nav>
    )
