...
function AdminUsersScreen({ initialUsers }: Props) {

    const [users, setUsers] = useState(initialUsers);
    const [checkAll, setcheckAll] = useState(false);

    return (
        <AdminscreenWrapper title="Users">
            ...
            <div className="w-full h-full overflow-x-scroll xl:overflow-x-hidden">
                <table className="min-w-full bg-white">
                    ...
                    <tbody>
                        {
                            users.map((user, key) => <TableUserLine
                                key={key}
                                user={user}
                                checkAll={checkAll}
                                updateMainUsers={setUsers}
                            />)
                        }
                    </tbody>
                </table>
...
export const getServerSideProps = async () => {
    ...
    await db.connect();
    const users = await User.find({}, { name: 1, email: 1, ordersHistory: 1, isAdmin: 1, createdAt: 1 }).lean();
    await db.disconnect();

    return {
        props: {
            initialUsers: users.map(db.convertDocToObj),
        },
    }
}
