...
import jsonFilePDGC from "@datassets/PDGC.json"

const Terms: NextPage = ({ jsonPDGC }) => {
    const pdgcTitle = "Politique de Confidentialité"
    const [pdgc, setPDGC] = useState(jsonPDGC[pdgcTitle]);
    ...
    const renderMenu = () => {
        return Object.keys(pdgc).map((key) => (
            <button
                key={key}
                onClick={() => {
                    scrollToArticle(key)
    ...
    return (
        <BasescreenWrapper title={pdgcTitle}>
            <div className="relative flex w-full h-full overflow-visible bg-gray-100 max-w-screen">
                ...
                <div className="w-full px-5 py-8 space-y-5 bg-white border-l-0 shadow-lg">
                    <h1 className="text-3xl font-bold">{pdgcTitle}</h1>
                    {pdgc && renderPDGC(pdgc)}
                </div>
